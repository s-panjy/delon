...
export abstract class FormProperty {
    ...
    constructor(
        ...
        this.schemaValidator = schemaValidatorFactory.createValidatorFn(schema, {
            ingoreKeywords: this.ui.ingore_keywords
        });
    ...
    updateValueAndValidity(onlySelf = false, emitValueEvent = true, emitValidator = true) {
        ...
        if (emitValidator && this.ui.live_validate === true) {
            this._runValidation();
        }
    ...
    protected setErrors(errors: ErrorData[], emitFormat = true) {
        if (emitFormat && errors && !this.ui.only_visual) {
            errors = errors.map((err: ErrorData) => {
                let message = err._custom === true && err.message ? err.message : (this.ui.errors || {})[err.keyword] || this.options.errors[err.keyword] || ``;
                ...
                if (message) {
                    if (~(message as string).indexOf('{')) {
                        message = (message as string).replace(/{([\.a-z0-9]+)}/g, (v: string, key: string) => err.params[key] || '');
                    }
    ...
    _bindVisibility() {
        ...
        if (typeof visibleIf === 'object' && Object.keys(visibleIf).length === 0) {
            ...
            for (const dependencyPath in visibleIf) {
                if (visibleIf.hasOwnProperty(dependencyPath)) {
                    ...
                    if (property) {
                        ...
                        const and = combineLatest([valueCheck, visibilityCheck], (v1: boolean, v2: boolean) => v1 && v2);
            ...
            combineLatest(propertiesBinding, (...values: boolean[]) => values.indexOf(true) !== -1)
